-- 003_affiliate_tracking_events.sql
-- Add event tracking for affiliate clicks, signups, and conversions

-- Create affiliate tracking events table
CREATE TABLE IF NOT EXISTS affiliate_events (
  id SERIAL PRIMARY KEY,
  referral_code VARCHAR(255) REFERENCES unified_referrals(referral_code),
...
CREATE INDEX IF NOT EXISTS idx_affiliate_events_processed ON affiliate_events(processed_at) WHERE processed_at IS NOT NULL;

-- Create affiliate click tracking table (for detailed analytics)
CREATE TABLE IF NOT EXISTS affiliate_clicks (
  id SERIAL PRIMARY KEY,
  referral_code VARCHAR(255) REFERENCES unified_referrals(referral_code),
  click_id VARCHAR(255) UNIQUE NOT NULL, -- unique identifier for this click
  ip_address INET,
  user_agent TEXT,
  referrer_url TEXT,
...
-- Function to calculate affiliate metrics
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(commissions.total, 0) as total_commission,
    COALESCE(commissions.available, 0) as available_balance,
    COALESCE(commissions.pending, 0) as pending_balance
  FROM
    (SELECT COUNT(*) as count FROM affiliate_clicks WHERE referral_code = referral_code_param) clicks,
    (SELECT COUNT(*) as count FROM affiliate_events WHERE referral_code = referral_code_param AND event_type = 'signup') signups,
    (SELECT COUNT(*) as count FROM affiliate_events WHERE referral_code = referral_code_param AND event_type = 'conversion') conversions,
    (SELECT
     ...
     FROM affiliate_commissions WHERE referral_code = referral_code_param) commissions;
END;
$$ LANGUAGE plpgsql;
