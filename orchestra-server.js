...
// Zero dependencies (stdlib only). Usage:  node orchestra-server.js [port]
//
...
const oauthGoogle = require('./lib/oauth-google');
...
function serveHtml(res){
  fs.readFile(HTML, (err, buf) => {
    if (err){
      ...
      return res.end('orchestra.html not found next to orchestra-server.js');
    }
...
http.createServer((req, res) => {
  ...
  if (u === '/affiliate/dashboard' || u === '/affiliate/dashboard.html'){
    const affiliatePath = path.resolve(ROOT, '../affiliate-portal/dashboard.html');
  ...
  // /auth/google/callback  -> verify, issue bridge_admin_session, 302 to /settings/admin
  // /auth/logout           -> clear session + oauth state cookies, 302 to /
  // Delegates to lib/oauth-google.js; enforces same super-admin allowlist
  // as POST /settings/session so only store.users[].tier==='super_admin'
  // can actually land a session.
  if (u === '/auth/google' && req.method === 'GET'){
    return oauthGoogle.handleStart(req, res, { adminToken: ADMIN_TOKEN, secure: COOKIE_SECURE });
  }
  if (u === '/auth/google/callback' && req.method === 'GET'){
    Promise.resolve(oauthGoogle.handleCallback(req, res, {
      adminToken: ADMIN_TOKEN, secure: COOKIE_SECURE, store: settingsStore,
    })).catch(err => {
      if (!res.headersSent){
        res.writeHead(500, {...HARDENING_HEADERS,'content-type':'application/json'});
        res.end(JSON.stringify({ok:false, error:'oauth_callback_failed', detail:String(err.message||err)}));
      }
    });
    return;
  }
  if (u === '/auth/logout'){
    return oauthGoogle.handleLogout(req, res);
  }

  // /api/affiliate/* and /ref/* — proxied to backend (real DB-backed handlers)
  if (u.startsWith('/api') || u.startsWith('/ref')) return proxy(req, res);
