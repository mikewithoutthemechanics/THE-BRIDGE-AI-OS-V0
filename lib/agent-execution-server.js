...
async function executeWithLLM(agentName, input, context, formatter) {
  ...
  if (ledger) {
    try {
      ledger.debit(`agent-exec-${agentName}`, AGENT_COST, 'execution', `Execution: ${input.slice(0, 80)}`);
  ...
  if (!llm) {
    ...
    if (ledger) {
      try { ledger.credit(`agent-exec-${agentName}`, AGENT_COST * 0.5, 'stub_completion', 'Stub execution credit'); } catch (_) {}
    }
  ...
  if (ledger) {
    try {
      ledger.credit(`agent-exec-${agentName}`, AGENT_COST * 1.5, 'successful_execution', `Completed: ${input.slice(0, 80)}`);
    } catch (_) {}
  }
...
function getAgentStatus(name) {
  ...
  let balance = null;
  if (ledger) {
    try { balance = ledger.getBalance(`agent-exec-${name}`); } catch (_) {}
  }
