...
async function ensureAgent(agentId) {
  if (!isConfigured || !supabase) return { agent_id: agentId, balance: 0, _offline: true };
...
async function credit(agentId, amount, type, memo) {
  if (!isConfigured || !supabase) return { tx_id: null, agent_id: agentId, amount, new_balance: 0, _offline: true };
  if (!type) type = 'credit';
  if (!memo) memo = '';
...
async function transfer(fromAgent, toAgent, amount, taskId) {
  if (!isConfigured || !supabase) return { tx_id: null, from: fromAgent, to: toAgent, amount, _offline: true };
  ...
  // ── Record platform fee to proof chain + system treasury ───────────────
  if (fee > 0) {
    try {
      ...
      await proofStore.recordPayment({
        ...
        source: 'a2a-transfer',
        webhookId: id,
        webhookSignature: { method: 'a2a_transfer', verified: true },
        timestamp: ts,
        meta: { from: fromAgent, to: toAgent, gross: amount, net, taskId },
      });
      await db.addToTreasury(fee, 'a2a_fee:' + id);
    } catch (_) {}
  }
...
async function getStats() {
  if (!isConfigured || !supabase) return { totalCirculating: 0, totalEarned: 0, totalSpent: 0, agentCount: 0, _offline: true };
  // Total supply
  ...
  // Counts — use agent_balances when seeded, else fall back to agents registry
  const { count: balanceAgentCount } = await supabase
    .from('agent_balances')
    .select('*', { count: 'exact', head: true });

  const { count: registryAgentCount } = await supabase
    .from('agents')
    .select('*', { count: 'exact', head: true });

  const agentCount = Math.max(balanceAgentCount || 0, registryAgentCount || 0);
  ...
  return {
    ...
    agentCount: agentCount,   // alias used by admin-revenue.html
    txCount: txCount || 0,
