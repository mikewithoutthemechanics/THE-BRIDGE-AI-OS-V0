...
db.exec(`
  CREATE TABLE IF NOT EXISTS agent_balances (
    ...
    last_tx       TEXT,
    fiat_revenue      REAL NOT NULL DEFAULT 0,
    ap2_revenue       REAL NOT NULL DEFAULT 0,
    affiliate_revenue REAL NOT NULL DEFAULT 0,
    fiat_cost         REAL NOT NULL DEFAULT 0
  );
...
// ── Migrate: add P&L columns if missing (existing DBs) ────────────────────
try {
  const cols = db.pragma('table_info(agent_balances)').map(c => c.name);
  if (!cols.includes('fiat_revenue')) db.exec('ALTER TABLE agent_balances ADD COLUMN fiat_revenue REAL NOT NULL DEFAULT 0');
  if (!cols.includes('ap2_revenue')) db.exec('ALTER TABLE agent_balances ADD COLUMN ap2_revenue REAL NOT NULL DEFAULT 0');
  if (!cols.includes('affiliate_revenue')) db.exec('ALTER TABLE agent_balances ADD COLUMN affiliate_revenue REAL NOT NULL DEFAULT 0');
  if (!cols.includes('fiat_cost')) db.exec('ALTER TABLE agent_balances ADD COLUMN fiat_cost REAL NOT NULL DEFAULT 0');
 ...
 * @param {string} agentId
 * @param {number} amount — must be > 0
 * @param {'fiat'|'ap2'|'affiliate'} type
 * @returns {{ agent_id, type, amount, new_total }}
 */
function recordRevenue(agentId, amount, type) {
  ...
  const run = db.transaction(() => {
    ...
    db.prepare(`UPDATE agent_balances SET ${col} = ? WHERE agent_id = ?`).run(newVal, agentId);
    // Also credit the BRDG balance
    credit(agentId, amount, 'revenue_' + type, type + ' revenue: ' + amount);
 ...
 * @param {string} agentId
 * @param {number} amount — must be > 0
 * @returns {{ agent_id, amount, new_total }}
 */
function recordCost(agentId, amount) {
  if (amount <= 0) throw new Error('Cost amount must be positive');

  const run = db.transaction(() => {
    const agent = ensureAgent(agentId);
    const newVal = round8((agent.fiat_cost || 0) + amount);
    db.prepare('UPDATE agent_balances SET fiat_cost = ? WHERE agent_id = ?').run(newVal, agentId);
 ...
 * @param {string} agentId
 * @returns {{ balance, fiat_revenue, ap2_revenue, affiliate_revenue, fiat_cost, net_profit }}
 */
function getAgentPnL(agentId) {
  const agent = ensureAgent(agentId);
 ...
 * @returns {{ total_fiat_revenue, total_ap2_revenue, total_affiliate_revenue, total_fiat_cost, total_net_profit, agent_count }}
 */
function getSystemPnL() {
  const row = db.prepare(`
    SELECT
      COUNT(*) AS agent_count,
      COALESCE(SUM(fiat_revenue), 0) AS total_fiat_revenue,
      COALESCE(SUM(ap2_revenue), 0) AS total_ap2_revenue,
      COALESCE(SUM(affiliate_revenue), 0) AS total_affiliate_revenue,
      COALESCE(SUM(fiat_cost), 0) AS total_fiat_cost
    FROM agent_balances
  `).get();

  return {
    total_fiat_revenue: row.total_fiat_revenue,
    total_ap2_revenue: row.total_ap2_revenue,
    total_affiliate_revenue: row.total_affiliate_revenue,
    total_fiat_cost: row.total_fiat_cost,
    total_net_profit: round8(row.total_fiat_revenue + row.total_ap2_revenue + row.total_affiliate_revenue - row.total_fiat_cost),
