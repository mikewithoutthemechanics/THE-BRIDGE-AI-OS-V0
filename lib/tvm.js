...
function sign(row) {
  return crypto.createHmac('sha256', getTVMSecret())
    .update(canonicalPayload(row)).digest('hex').slice(0, 32);
}
...
const RECOMMENDATION_LIB = {
  ...
  'VPS-RESTART-GW': {
    ...
    steps: [
      ...
      'SSH: ssh root@${VPS_IP} (set VPS_IP env var; do not hardcode)',
  ...
  'VPS-SSH-RESTORE': {
    ...
    steps: [
      'Email support@webway.co.za with subject: SSH Key Restoration — Server ${VPS_IP} (set VPS_IP env var)',
      'Attach your SSH public key (~/.ssh/id_rsa.pub)',
      'Wait for Webway to add to /root/.ssh/authorized_keys',
      'Test: ssh root@${VPS_IP}',
...
function updateRow(topic, agentUpdate, callerRole) {
  const matrix = getMatrix();
  const idx = matrix.findIndex(r => r.topic === topic);
  if (idx === -1) return { ok: false, error: 'topic not found' };

  const prev = { ...matrix[idx] };
  // Role must come from a trusted caller parameter, never from the update payload
