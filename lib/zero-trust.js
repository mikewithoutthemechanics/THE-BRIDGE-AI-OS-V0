...
function getMasterSecret() {
  const s = process.env.BRIDGE_VERIFY_SECRET || process.env.BRIDGE_INTERNAL_SECRET || process.env.BRIDGE_SIWE_JWT_SECRET;
  if (!s) {
    console.warn('[zero-trust] No signing secret found — using ephemeral key (signatures not stable across restarts)');
    return 'ephemeral-' + (process.env.VERCEL_DEPLOYMENT_ID || 'local');
  }
  return s;
}
...
function hashTransaction(tx, previousHash = '0'.repeat(64)) {
  // Normalize timestamp: Supabase returns +00:00 but JS uses Z — both mean UTC
  const rawTs = tx.timestamp || tx.created_at || '';
  const normalizedTs = rawTs.replace('+00:00', 'Z').replace(/\.(\d{3})\d*Z/, '.$1Z');
  const canonical = [
