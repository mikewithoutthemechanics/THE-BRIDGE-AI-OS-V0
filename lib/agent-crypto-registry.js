...
async function _sbInsert(record) {
...
async function ensureWallet(agentId, agentName = '') {
  ...
  if (existing) {
    _memStore.set(agentId, existing);
    return existing;
  }

  // 2. Check memory fallback
  if (_memStore.has(agentId)) return _memStore.get(agentId);
  ...
  try {
    ...
    // Master secret not set — return a placeholder so the rest of the system works
  ...
  // 4. Persist
  const saved = await _sbInsert(record);
  if (!saved) {
    console.warn(`[agent-crypto-registry] Supabase unavailable — using in-memory store for ${agentId}`);
  }
  _memStore.set(agentId, record);

  console.log(`[agent-crypto-registry] Wallet created for agent "${agentId}" → ETH: ${addresses.eth ?? 'n/a'} | BTC: ${addresses.btc ?? 'n/a'}`);
  return record;
}
