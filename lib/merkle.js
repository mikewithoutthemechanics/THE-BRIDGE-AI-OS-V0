...
/** Hash for leaf nodes: prefix 0x00 for domain separation (second-preimage resistance) */
function hashLeaf(data) {
  return crypto.createHash('sha256').update('\x00' + String(data)).digest('hex');
}

/** Hash for internal nodes: prefix 0x01 for domain separation (second-preimage resistance) */
function hashInternal(left, right) {
  return crypto.createHash('sha256').update('\x01' + left + right).digest('hex');
}
...
class MerkleTree {
  ...
  insert(leafData) {
    this.leaves.push(hashLeaf(leafData));
    this._build();
    return this.root;
  }

  /**
   * Append a pre-hashed leaf value (e.g. from auth-merkle audit log).
   * Use this instead of directly mutating this.leaves.
  ...
  verify(leafData, proof, expectedRoot) {
    ...
    for (const step of proof) {
      hash = step.position === 'right'
        ? hashInternal(hash, step.hash)
        : hashInternal(step.hash, hash);
    }
...
module.exports = { MerkleTree, sha256, hashLeaf, hashInternal };
