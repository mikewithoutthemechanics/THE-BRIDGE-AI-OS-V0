...
// ── State ────────────────────────────────────────────────────────────────────
let _running = false;
let _lastLeadAt = null; // ISO timestamp cursor for delta crm_leads queries
let _stats = {
  loopsCompleted: 0,
  leadsGenerated: 0,
  leadsQualified: 0,
...
async function generateLeads() {
  // Pull real leads from Supabase crm_leads (status='new', created since last tick).
  // Also compatible with no-leads state — returns [] without erroring.
  const leads = [];
  ...
  try {
    ...
    if (isConfigured && supabaseAdmin) {
      let query = supabaseAdmin
        .from('crm_leads')
        .select('id, email, score, company, source, status, created_at')
        .eq('status', 'new')
        .order('created_at', { ascending: true })
        .limit(50);

      // Delta: only fetch leads newer than the last cursor position
      if (_lastLeadAt) {
        query = query.gt('created_at', _lastLeadAt);
      }
      ...
      if (!error && Array.isArray(rows) && rows.length > 0) {
        for (const row of rows) {
          leads.push({
            ...
            source: row.source || 'crm_leads',
            createdAt: row.created_at,
          });
        }
        // Advance cursor so next tick fetches only newer leads
        _lastLeadAt = rows[rows.length - 1].created_at;
      } else if (error) {
        console.warn('[REVENUE-ENGINE] crm_leads query error:', error.message);
      }
    }
  } catch (e) {
    console.warn('[REVENUE-ENGINE] generateLeads error:', e.message);
  }
...
async function convertLeads(qualified) {
  // Mark qualified leads as 'converted' in crm_leads and produce deal objects.
      ...
      try {
        if (isConfigured && supabaseAdmin) {
          const { error } = await supabaseAdmin
            ...
            .eq('id', lead.id)
            .eq('status', 'new'); // idempotent: only if still new
          if (error) {
            console.warn('[REVENUE-ENGINE] crm_leads update error:', error.message);
            continue;
          }
        }
        conversions.push({
          ...
          region: lead.region,
          source: 'crm_auto_convert',
        });
      } catch (_) {}
    }
  } catch (e) {
    console.warn('[REVENUE-ENGINE] convertLeads error:', e.message);
  }
  return conversions;
}
...
async function recordRevenue(deal) {
  try {
    ...
    // Write fiat_revenue to the agent ledger (prime-001 is the revenue-recording agent)
    try {
      const ledger = require('./agent-ledger');
      await ledger.recordRevenue('prime-001', deal.amount, 'fiat');
    } catch (_) {}
...
async function autoCreateMarketplaceTasks() {
  try {
    ...
    // Pick the next task in rotation (no randomness — deterministic round-robin)
    var shuffled = MARKETPLACE_TASKS;
    var count = 1;
    var created = 0;
...
function start(intervalMs) {
  if (process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID) {
    return { ok: false, error: 'Revenue engine is disabled under NODE_ENV=test' };
  }
