...
async function recordRevenue(deal) {
  try {
    ...
    recordRevenueHistory(deal.amount, deal.source);
    await checkMilestones(_stats.revenueGenerated);
...
function recordRevenueHistory(amount, source) {
  const entry = {
    amount,
    source,
    timestamp: new Date().toISOString(),
    runningTotal: _stats.revenueGenerated,
  };
  _revenueHistory.unshift(entry);
  if (_revenueHistory.length > MAX_HISTORY) {
    _revenueHistory = _revenueHistory.slice(0, MAX_HISTORY);
  }
  try {
    db.setState('revenue_history', _revenueHistory);
  } catch (_) {}
}

function getRevenueHistory(limit = 50) {
  return _revenueHistory.slice(0, limit);
}

// ── Milestone Notifications ──────────────────────────────────────────────────

async function checkMilestones(totalRevenue) {
  for (const milestone of MILESTONES) {
    if (totalRevenue >= milestone && !_notifiedMilestones.has(milestone)) {
      _notifiedMilestones.add(milestone);
      const msg = `🎉 <b>Revenue Milestone Reached!</b>\nTotal Revenue: R${totalRevenue.toLocaleString()}\nMilestone: R${milestone}`;
      notify.alertSystemEvent(msg);
      console.log(`[REVENUE-ENGINE] 🎉 Milestone reached: R${milestone}`);
      try {
        await db.setState('notified_milestones', Array.from(_notifiedMilestones));
      } catch (_) {}
    }
  }
}
...
function getMilestones() {
  return {
    milestones: MILESTONES.map(m => ({
      amount: m,
      reached: _stats.revenueGenerated >= m,
      notified: _notifiedMilestones.has(m),
    })),
    currentTotal: _stats.revenueGenerated,
  };
}
...
async function startup() {
  try {
    const savedHistory = await db.getState('revenue_history');
    if (savedHistory && Array.isArray(savedHistory)) {
      _revenueHistory = savedHistory;
    }
    const savedMilestones = await db.getState('notified_milestones');
    if (savedMilestones && Array.isArray(savedMilestones)) {
      _notifiedMilestones = new Set(savedMilestones);
    }
    const savedStats = await db.getState('revenue_engine_stats');
    if (savedStats) {
      _stats = { ..._stats, ...savedStats };
    }
  } catch (_) {}

  start(60000);
  console.log('[REVENUE-ENGINE] Auto-started on server boot');
}
