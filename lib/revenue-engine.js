...
async function generateLeads() {
  // Leads come from real sources (CRM, webhook signups, form submissions).
  // We do NOT generate synthetic leads — that would inflate pipeline metrics.
  // Future: query CRM table for new signups since last tick.
  const leads = [];

  try {
    await db.setState('revenue_engine_leads', {
      total: _stats.leadsGenerated,
      lastBatch: 0,
...
async function convertLeads(qualified) {
  // Conversions are recorded by real payment webhooks (Stripe, PayFast, etc.)
  // arriving via /webhooks/stripe or /webhooks/payfast — not synthesised here.
  // This function is a no-op until real CRM lead data is wired in.
  return [];
}
