...
const DA_BASE    = (process.env.DA_BASE_URL || 'https://mail.api.ai-os.co.za:2222').replace(/\/$/, '');
...
// ── DirectAdmin CGI form client (email API uses URL-encoded, not JSON) ─────────

function daFormFetch(path, params = {}, method = 'GET') {
  return new Promise((resolve, reject) => {
    const auth = authHeader();
    if (!auth) return reject(new Error('DA_USERNAME / DA_LOGIN_KEY not configured'));
    ...
    const url    = new URL(DA_BASE + path + (!isPost && qs ? '?' + qs : ''));
    const lib    = url.protocol === 'https:' ? https : http;
    const body   = isPost ? Buffer.from(qs, 'utf8') : null;
    ...
    const req = lib.request(opts, (r) => {
      ...
      r.on('data', d => { data += d; });
      r.on('end', () => {
        // DA returns key=value pairs separated by & or newlines
        ...
        resolve({ status: r.statusCode, raw: data, parsed });
      });
    });

    req.on('error', reject);
    req.setTimeout(12000, () => req.destroy(new Error('DA email request timeout')));
 ...
 * DA endpoint: GET /CMD_API_POP?domain=example.com
 */
async function listEmailAccounts(domain) {
  if (!isConfigured()) return { configured: false };
  const r = await daFormFetch('/CMD_API_POP', { domain }, 'GET');
  // Response: list=user1%0Auser2%0A...
  const list = (r.parsed.list || '').split('\n').filter(Boolean);
  return { ok: r.status === 200, domain, accounts: list, raw: r.parsed };
}

/**
 * Create a new email account.
 * DA endpoint: POST /CMD_API_POP action=create
 * @param {string} domain  - e.g. bridge-ai-os.com
 * @param {string} user    - local part, e.g. "admin" → admin@bridge-ai-os.com
 * @param {string} passwd  - password (min 6 chars)
 * @param {number} quota   - mailbox quota in MB (0 = unlimited)
 */
async function createEmailAccount(domain, user, passwd, quota = 250) {
  if (!isConfigured()) throw new Error('DirectAdmin not configured');
  if (!domain || !user || !passwd) throw new Error('domain, user, passwd required');
  if (passwd.length < 6) throw new Error('Password must be at least 6 chars');

  const r = await daFormFetch('/CMD_API_POP', {
    action: 'create',
    domain,
    user,
    passwd,
  ...
  const ok = r.parsed.error === '0' || r.status === 200;
  return {
    ok,
    email:   `${user}@${domain}`,
    message: r.parsed.text || r.raw.slice(0, 200),
    details: r.parsed.details || '',
  };
}

/**
 * Delete an email account.
 * DA endpoint: POST /CMD_API_POP action=delete
 */
async function deleteEmailAccount(domain, user) {
  if (!isConfigured()) throw new Error('DirectAdmin not configured');
  const r = await daFormFetch('/CMD_API_POP', {
    action:  'delete',
    domain,
  ...
  const ok = r.parsed.error === '0' || r.status === 200;
  return { ok, email: `${user}@${domain}`, message: r.parsed.text || r.raw.slice(0, 200) };
}

/**
 * Create an email forwarder.
 * DA endpoint: POST /CMD_API_EMAIL_FORWARDERS action=create
 * @param {string} domain
 * @param {string} user   - local part to forward FROM
 * @param {string} email  - forward TO this address
 */
async function createForwarder(domain, user, email) {
  if (!isConfigured()) throw new Error('DirectAdmin not configured');
  const r = await daFormFetch('/CMD_API_EMAIL_FORWARDERS', {
    action: 'create',
    domain,
    user,
    email,
  ...
  const ok = r.parsed.error === '0' || r.status === 200;
  return { ok, from: `${user}@${domain}`, to: email, message: r.parsed.text || r.raw.slice(0, 200) };
}

/**
 * Batch-create multiple email accounts for Bridge AI OS profiles.
 * Profiles: admin, content, gateway, support, noreply
 *
 * @param {string} domain    - e.g. 'bridge-ai-os.com'
 * @param {string} passwd    - shared password (change per-account after setup)
 * @param {string[]} users   - local-part names, defaults to Bridge AI OS profile set
 */
async function createBridgeProfiles(domain, passwd, users = null) {
  const defaults = ['admin', 'content', 'support', 'noreply', 'gateway'];
  const list     = users || defaults;

  const results = [];
  for (const user of list) {
    try {
      const r = await createEmailAccount(domain, user, passwd);
      results.push(r);
    } catch (e) {
      results.push({ ok: false, email: `${user}@${domain}`, message: e.message });
    }
  }
  ...
  return {
    ...
    domain,
    created: results.filter(r => r.ok).map(r => r.email),
