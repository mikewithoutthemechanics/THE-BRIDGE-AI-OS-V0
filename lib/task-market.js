...
async function completeTask(taskId, result) {
  ...
  // Atomic CAS (2026-04-18): only one concurrent caller flips CLAIMED/EXECUTING -> COMPLETED.
  // Without this, two overlapping AUTO-ECON sweeps both pass the status check,
  // then the loser hits "Not enough escrowed" because the winner already released it.
  ...
  if (casErr) throw new Error('completeTask CAS failed: ' + casErr.message);
  if (!casRows || casRows.length === 0) {
    const { data: current } = await supabase
      .from('tasks_market').select('status').eq('id', taskId).single();
    if (!current) throw new Error('Task not found');
    throw new Error('Cannot complete: task is ' + current.status);
  }

  const task = casRows[0];
