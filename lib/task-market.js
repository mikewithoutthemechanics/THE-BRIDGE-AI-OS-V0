...
function db() {
  ...
  try {
    _db.exec("ALTER TABLE tasks_market ADD COLUMN source TEXT NOT NULL DEFAULT 'internal'");
  } catch (_) {
 ...
 * @param {object} [opts]      - Optional params: { source: 'internal'|'external' }
 * @returns {object} The created task row
 */
function postTask(posterAgent, title, description, rewardBrdg, opts) {
  ...
  // Determine task source: 'internal' (agent-to-agent) or 'external' (paying user)
  var source = (opts && opts.source === 'external') ? 'external' : 'internal';

  const id = genTaskId();
  const d = db();
  d.prepare(
    'INSERT INTO tasks_market (id, poster_agent, title, description, reward_brdg, escrow_amount, status, source, posted_at) ' +
    "VALUES (?, ?, ?, ?, ?, ?, 'POSTED', ?, ?)"
  ).run(id, posterAgent, title, description || '', rewardBrdg, rewardBrdg, source, now());
