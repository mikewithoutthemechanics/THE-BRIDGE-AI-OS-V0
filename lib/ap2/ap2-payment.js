...
async function processPayment(offer) {
  ...
  // ── Record to proof chain for zero-trust dashboard visibility ──────────
  try {
    ...
    await proofStore.recordPayment({
      ...
      source: 'ap2-payment',
      webhookId: id,
      webhookSignature: { method: 'ap2_settlement', verified: true },
      timestamp: payment.settled_at,
      meta: { offerId: offer.offer_id, from: offer.from_agent, to: offer.to_agent },
    });
    // If settled via escrow (no ledger.transfer()), the 5% fee wasn't auto-deducted
    if (!transferResult) {
      const platformSlice = Math.round(payment.amount_brdg * 0.05 * 100) / 100;
      await db.addToTreasury(platformSlice, 'ap2_fee:' + id);
    }
  } catch (_) {}
