...
function buildPaymentUrl({ amount, email, itemName, firstName = 'Client', meta = '' }) {
  ...
  if (!merchantId || !merchantKey) {
    throw new Error('PayFast not configured: PAYFAST_MERCHANT_ID and PAYFAST_MERCHANT_KEY required');
  }
  const passphrase  = process.env.PAYFAST_PASSPHRASE   || '';
  const baseUrl     = process.env.NEXT_PUBLIC_BASE_URL  || 'https://bridge-ai-os.com';
