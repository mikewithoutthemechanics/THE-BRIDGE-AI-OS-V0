 ...
 * Replaces lib/supabase.js — no external dependency needed
 *
 * Reads/writes secrets from secrets_vault table in users.db
 * Falls back to env vars if secret not found in DB
...
function deleteSecret(keyName) {
  const d = getDB();
  d.prepare("DELETE FROM secrets_vault WHERE key_name = ?").run(keyName);
  delete cache[keyName.toUpperCase()];
}

/**
 * Sync secrets from Notion webhook payload
 * Called when Notion Secrets Vault database is updated
 */
 ...
 * Run once on first deploy to populate secrets_vault
 */
