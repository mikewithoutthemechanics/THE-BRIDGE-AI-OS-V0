...
function registerPrimeRoutes(app) {
  ...
  app.get('/api/primes', async (_req, res) => {
    try {
  ...
  app.get('/api/primes/:id', async (req, res) => {
    try {
  ...
  app.post('/api/primes/:id/delegate', async (req, res) => {
    try {
  ...
  app.get('/api/primes/:id/subordinates', async (req, res) => {
    try {
      const prime = primes.getPrimeAgent(req.params.id);
      if (!prime) return res.status(404).json({ ok: false, error: 'Prime agent not found' });
