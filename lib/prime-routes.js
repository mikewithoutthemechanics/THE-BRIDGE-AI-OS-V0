...
function registerPrimeRoutes(app) {
  ...
  app.get('/api/primes',
    validation.validateRequest({
      ...
      offset: { type: 'positive', required: false, default: 0 }
    }),
    async (req, res) => {
      try {
        ...
        const enriched = await Promise.all(agents.map(async (p) => {
          let balance = null;
          if (ledger) {
            try {
              const bal = await ledger.getBalance(p.id);
  ...
  app.get('/api/primes/stats', async (req, res) => {
    try {
  ...
  app.get('/api/primes/:id',
    ...
    async (req, res) => {
      try {
        ...
        if (ledger) {
          try {
            const bal = await ledger.getBalance(prime.id);
  ...
  app.post('/api/primes/:id/think',
    validation.validateRequest({
      id: { type: 'string', required: true, validate: validation.validateId },
      question: { type: 'string', required: true, min: 1, max: 5000 }
    }),
    async (req, res) => {
      try {
  ...
  app.post('/api/primes/:id/delegate',
    validation.validateRequest({
      id: { type: 'string', required: true, validate: validation.validateId },
    ...
    async (req, res) => {
      try {
        ...
        const result = await primes.delegateTask(req.params.id, { title, description, reward });
        res.json({ ok: true, ...result });
      } catch (e) {
        res.status(e.message.includes('not found') ? 404 : 500).json({ ok: false, error: e.message });
      }
  ...
  app.get('/api/primes/:id/subordinates',
    validation.validateRequest({
      id: { type: 'string', required: true, validate: validation.validateId }
    }),
    async (req, res) => {
      try {
        ...
        const subordinates = await Promise.all(prime.subordinates.map(async (id) => {
          let balance = null;
          if (ledger) {
            try {
              const bal = await ledger.getBalance(id);
              balance = { balance: bal.balance, earned: bal.earned_total, spent: bal.spent_total };
            } catch (_) {}
          }
          return { id, balance };
