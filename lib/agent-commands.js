...
function registerAgentCommands(app) {
  ...
  // Allows unauthenticated demo access (rate-limited to 5 per minute per IP)
  ...
  app.post('/api/agents/:id/command', async (req, res) => {
    ...
    if (adminSecret && adminSecret === process.env.BRIDGE_INTERNAL_SECRET) {
      ...
      if (entry.count > 5) return res.status(429).json({ ok: false, error: 'Demo rate limit: 5 commands/minute. Sign up for unlimited access.' });
      auth = { role: 'demo' };
    }
