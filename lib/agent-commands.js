...
async function getAgentBalance(agentId) {
  ...
  try {
    return await ledger.getBalance(agentId);
  } catch (_) {
    return { balance: 0, earned_total: 0, spent_total: 0, escrowed: 0 };
  }
}

// Demo rate limiter (module scope — shared by gateway + serverless)
const demoLimiter = new Map();

/**
 * POST /api/agents/:id/command — core handler (no Express).
 * Used by gateway + Vercel so "Try it" works when brain is unreachable.
 */
async function executeAgentCommandPost({ agentId, body, headers, remoteIp }) {
  const h = headers || {};
  const fwd = (h['x-forwarded-for'] || '').toString().split(',')[0].trim();
  const ip = fwd || remoteIp || 'unknown';

  let auth = null;
  const adminSecret = h['x-bridge-secret'];
  const apiKey = h['x-api-key'];
  if (adminSecret && adminSecret === process.env.BRIDGE_INTERNAL_SECRET) {
    auth = { role: 'admin' };
  } else if (apiKey && apiKey.length > 10) {
    auth = { role: 'user', key: apiKey };
  } else {
    const now = Date.now();
    const entry = demoLimiter.get(ip) || { count: 0, reset: now + 60000 };
    if (now > entry.reset) { entry.count = 0; entry.reset = now + 60000; }
    entry.count++;
    demoLimiter.set(ip, entry);
    if (entry.count > 5) {
      return { status: 429, payload: { ok: false, error: 'Demo rate limit: 5 commands/minute. Sign up for unlimited access.' } };
    }
    auth = { role: 'demo' };
  }
  ...
  if (!command || typeof command !== 'string') {
    return { status: 400, payload: { ok: false, error: 'command (string) is required in body' } };
  }

  const agent = findAgent(agentId);
  if (!agent) {
    return { status: 404, payload: { ok: false, error: 'Agent not found: ' + agentId } };
  }

  if (!llm) {
    return { status: 503, payload: { ok: false, error: 'LLM client not available — cannot execute agent commands' } };
  }

  const bal = await getAgentBalance(agentId);
  const skillsList = (agent.skills || []).join(', ');
  const fromWho = auth.role === 'admin' ? 'an admin' : auth.role === 'demo' ? 'a demo visitor' : 'a user';
  const systemPrompt = 'You are ' + agent.name + ', a ' + (agent.role || 'general') +
    ...
    '. You are responding to a direct command from ' + fromWho +
  ...
  try {
    ...
    return { status: 500, payload: { ok: false, error: 'LLM inference failed: ' + err.message } };
  }
}
...
// ── 2. POST /api/agents/:id/task — Make an agent post a task ───────────────
  app.post('/api/agents/:id/task',
    validation.validateRequest({
      id: { type: 'string', required: true, validate: validation.validateId },
      title: { type: 'string', required: true, min: 1, max: 200 },
      description: { type: 'string', required: false, max: 2000 },
      reward: { type: 'positive', required: true }
    }),
    async (req, res) => {
      const { id } = req.params;
      ...
      try {
        const result = await market.postTask(id, title, description, reward);
  ...
  app.get('/api/agents/:id/status', async (req, res) => {
    ...
    let recentTx = [];
    if (ledger) {
      try { recentTx = await ledger.getHistory(agentId, 10); } catch (_) { /* no history */ }
    }

    // Active tasks
    let tasks = [];
    if (market) {
      try { tasks = await market.getAgentTasks(agentId); } catch (_) { /* no tasks */ }
    }
  ...
  app.get('/api/agents/roster', async (_req, res) => {
    const agents = getAgents();
...
module.exports = { registerAgentCommands, executeAgentCommandPost };
