...
let banks;
try { banks = require('./banks'); } catch (_) { banks = null; }

let actionLogger = null;
try { actionLogger = require('./agent-action-logger'); } catch (_) {}

const SPAWN_CREDIT = 0.05;
...
async function register(agent) {
  ...
  if (banks && SPAWN_CREDIT > 0) {
    try {
      await banks.creditBank('ops', SPAWN_CREDIT, 'agent_spawn', `New agent spawned: ${name}`, { agent_id: id });
    } catch (e) {
      console.warn('[agent-registry] bank credit failed:', e.message);
    }
  }

  // Auto-provision crypto wallet for every new agent (non-blocking)
  try {
    const cryptoReg = require('./agent-crypto-registry');
    cryptoReg.ensureWallet(id, name).catch(e =>
      console.warn('[agent-registry] crypto wallet creation deferred:', e.message)
    );
  } catch (_) {}

  // Audit log
  if (actionLogger) {
    actionLogger.log(id, 'register', {
      name: row.name, role: row.role, layer: row.layer, type: row.type,
      uuid: inserted?.uuid,
    }, { actor: 'system' }).catch(() => {});
  }
...
async function update(id, changes) {
  ...
  if (actionLogger) {
    actionLogger.log(id, 'update', { changes: Object.keys(updates) }, { actor: 'system' }).catch(() => {});
  }
...
async function remove(id) {
  ...
  if (actionLogger) {
    actionLogger.log(id, 'remove', { name: agent.name, role: agent.role }, { actor: 'system' }).catch(() => {});
  }
...
var SEED_AGENTS = [
  ...
  // === BAN NODE AGENTS ===
  { id: 'ban-ryan',            name: 'Ryan',         role: 'founder_execution',      layer: 'ban', type: 'founder', source: 'banks.js', skills: ['task_execution', 'high_trust', 'ban_orchestration'], config: { founder_bank: 'founder', layer: 'ban' } },
  ...
  { id: 'ban-marvin',          name: 'Marvin',       role: 'founder_consolidator',    layer: 'L3', type: 'founder', source: 'banks.js', skills: ['code_consolidation', 'performance_optimization', 'query_optimization', 'parallel_orchestration'], config: { laptop: 'L3', port: 9002, instances: 4, founder_bank: 'founder' } },

  // === BANKING AGENTS (Michael L2, Marvin L3) ===
  { id: 'ban-michael',         name: 'MikeBank',     role: 'founder_banking',         layer: 'L2', type: 'founder', source: 'banks.js', skills: ['payment_processing', 'escrow_management', 'treasury_ops', 'settlement', 'founder_finance'], config: { founder_bank: 'founder', layer: 'L2' } },
