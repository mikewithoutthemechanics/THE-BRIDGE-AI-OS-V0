...
//
// MUST stay byte-for-byte compatible with aoe-unified-final-main/lib/session.js
// so sessions issued by the Orchestra proxy (POST /settings/session) are
// verifiable here in services/admin-api. The signing algorithm (HMAC-SHA256)
// and cookie names are part of the wire contract — do not diverge.
//
// Design:
...
//     classic double-submit pattern.
//   - Secret derivation: ORCHESTRA_SESSION_SECRET wins, else derived from
//     ORCHESTRA_ADMIN_TOKEN, else a random per-boot fallback. The per-boot
//     fallback is fine on the issuer (operator re-logs-in) but it means that
//     verifier processes MUST share at least one of the env vars with the
//     issuer, or every verification will fail.
