 ...
 *  - PBX extension + number management
 *  - AI-powered lead nurture (Claude claude-sonnet-4-6)
 *  - Billing and wallet top-ups
 *  - Call records and AI call summaries
 */

const { supabase, isConfigured } = require('./supabase');

// ─── AI Client (Anthropic claude-sonnet-4-6 → OpenRouter fallback) ──────────────
const ANTHROPIC_KEY = process.env.ANTHROPIC_API_KEY || '';
const OR_KEY        = process.env.OPENROUTER_API_KEY || '';

async function callAI(systemPrompt, userMessage, opts = {}) {
  // Try Anthropic claude-sonnet-4-6 first (elite tier)
  if (ANTHROPIC_KEY) {
    try {
      ...
      if (data.content?.[0]?.text) return { ok: true, text: data.content[0].text, model: 'claude-sonnet-4-6' };
    } catch (e) { /* fall through */ }
  }

  // OpenRouter fallback (claude-3-haiku via OR)
  if (OR_KEY) {
    try {
      ...
      const data = await resp.json();
      if (data.choices?.[0]?.message?.content) return { ok: true, text: data.choices[0].message.content, model: 'claude-3-haiku-or' };
    } catch (e) { /* fall through */ }
  }

  // Stub fallback — always returns valid structured data
  return {
    ok: true, model: 'stub',
    text: JSON.stringify({
      score: 72,
