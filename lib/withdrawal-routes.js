...
let _withdrawalTableChecked = false;
async function ensureWithdrawalTable() {
  if (_withdrawalTableChecked) return;
  _withdrawalTableChecked = true;
  ...
  if (error && error.code === '42P01') {
    console.warn('[withdrawal-routes] withdrawal_requests table missing — attempting auto-create...');
    try {
      ...
      if (supabaseAdmin) {
        ...
        console.log('[withdrawal-routes] Auto-created withdrawal_requests table successfully');
      }
    } catch (createErr) {
      console.warn('[withdrawal-routes] Auto-create failed:', createErr.message);
    }
  }
}
