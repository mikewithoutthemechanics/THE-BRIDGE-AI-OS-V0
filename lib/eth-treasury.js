...
function getWallet() {
  if (!_wallet) {
    const key = '0x' + deriveTreasuryKey(masterSecret()).toString('hex');
...
async function withdraw(to, amount) {
  ...
  // Safety: check balance before sending
  const bal = await getProvider().getBalance(getWallet().address);
  if (bal < value) {
    throw new Error(`Insufficient balance: have ${ethers.formatEther(bal)} ETH, need ${amount} ETH`);
  }
