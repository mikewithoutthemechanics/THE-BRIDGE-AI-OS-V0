...
const queues = {
  ...
  goal: new Queue('goal-processing', {
    connection: redisClient,
    defaultJobOptions: {
      removeOnComplete: 10,
      removeOnFail: 5,
  ...
  architect: new Queue('architect-planning', {
    connection: redisClient,
    defaultJobOptions: {
      removeOnComplete: 20,
      removeOnFail: 10,
...
Object.keys(queues).forEach(name => {
  try {
    schedulers[name] = new QueueScheduler(name + '-scheduler', {
      connection: redisClient,
    });
  } catch (e) {
    console.warn(`[QUEUE] Scheduler for ${name} not available:`, e.message);
  }
});
