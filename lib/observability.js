...
// Admin-gate middleware for Bull Board. Matches admin-api/index.js token semantics.
function bullBoardAuth(req, res, next) {
  const token = process.env.ADMIN_API_TOKEN || process.env.BRIDGE_INTERNAL_SECRET;
  if (!token || token.length < 16) {
    return res.status(503).send('Bull Board disabled: ADMIN_API_TOKEN not configured');
  }
  const header = req.headers.authorization || '';
  const supplied = header.startsWith('Bearer ')
    ? header.slice(7)
    : (req.headers['x-admin-token'] || req.query.token || '');
  if (!supplied) return res.status(401).send('unauthorized');
...
function setupBullBoard(app) {
  ...
  try {
    ...
    if (adapters.length > 0) {
      ...
      // Mount Bull Board at /admin/queues — auth gate first, then router
      app.use('/admin/queues', bullBoardAuth, serverAdapter.getRouter());

      console.log('[OBSERVABILITY] Bull Board mounted at /admin/queues (auth required)');
    } else {
