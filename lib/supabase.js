...
// Treat placeholder values as unconfigured — URL must be a real Supabase project ref
const isConfigured = !!(
  SUPABASE_URL &&
  ...
  !SUPABASE_SERVICE_KEY.includes('your-service-role-key')
);

if (!isConfigured) {
  console.warn('[SUPABASE] Not configured — running in OFFLINE MODE (agent economy, revenue, and DB writes will noop).');
}
...
module.exports = { supabase, supabaseAdmin: supabase, supabaseAnon, isConfigured, SUPABASE_URL };
