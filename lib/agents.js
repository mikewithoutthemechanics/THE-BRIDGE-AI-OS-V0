...
const AGENT_DEFS = {
  ...
  'Infra AI': {
    system: 'You are a self-improving SRE AI. You analyze infrastructure metrics AND learn from the outcomes of past actions. Produce: (1) health_score 0-100, (2) anomalies list, (3) top 3 recommended actions ranked by success probability based on historical outcomes. Mark actions needing human approval. Be terse and numeric.',
    inputPrompt: (input) => `VPS snapshot + outcome history: ${input || 'no snapshot yet'}. Return ops report with health_score, anomalies, and ranked_actions.`,
  },
};
...
// ── Value-positive validation ─────────────────────────────────────────────────

const MIN_OUTPUT_CHARS = 80; // below this is noise, not intelligence

/**
 * Returns true if a single agent result is worth persisting.
 * Rule: no error, output exists, output is substantive.
 */
function isValuePositive(result) {
  if (!result)                    return false;
  if (result.error)               return false;
  if (!result.output)             return false;
  if (result.output.trim().length < MIN_OUTPUT_CHARS) return false;
  return true;
}

/**
 * Run all agents. Returns { results, valid, discarded, executionStatus }
 * Only value-positive outputs are flagged for persistence.
 * executionStatus: 'complete' | 'partial' | 'failed'
 */
async function runAllAgentsValidated() {
  ...
  const valid     = results.filter(r => isValuePositive(r));
  const discarded = results.filter(r => !isValuePositive(r));

  let executionStatus;
  if (valid.length === results.length)    executionStatus = 'complete';
  else if (valid.length > 0)              executionStatus = 'partial';
  else                                    executionStatus = 'failed';

  return { results, valid, discarded, executionStatus };
}

module.exports = { runAgent, runAllAgents, runAllAgentsValidated, runAgents, AGENT_DEFS, isValuePositive };
