    ...
    for (let i = 0; i < events.length; i += batchSize) {
      ...
      for (const event of batch) {
        try {
          ...
          // Fire on-chain BRDG distribution using the calculated reward amount
          if (reward > 0) {
            try {
              ...
              if (userRow?.wallet_address) {
                const { distributeAmount } = require('./brdg-distributor');
                distributeAmount(userRow.wallet_address, reward, eventType).catch(e =>
