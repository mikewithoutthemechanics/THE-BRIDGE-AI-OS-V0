    ...
    for (let i = 0; i < events.length; i += batchSize) {
      ...
      for (const event of batch) {
        try {
          ...
          // Fire on-chain BRDG distribution if user has a registered wallet
          try {
            const { data: userRow } = await supabase
              ...
              .eq('id', event.user_id)
              .single();
            if (userRow && userRow.wallet_address) {
              const { distributeReward: brdgSend } = require('./brdg-distributor');
              brdgSend(userRow.wallet_address, 'run_completed').catch(e =>
                console.warn('[Rewards] on-chain BRDG send failed:', e.message)
              );
            }
          } catch (_) {}
...
async function getRewardStats(eventType, options = {}) {
  try {
    ...
    const { data: rewardedData, error: rewardedError } = await supabase
      ...
      .select('count,sum(reward_amount)::numeric,max(rewarded_at)')
      .eq('event_type', eventType)
      .not('rewarded_at', 'is', null);

    // Get pending/unrewarded
    ...
    const lastHourStart = new Date(Date.now() - 60 * 60 * 1000).toISOString();
    const { data: hourlyData, error: hourlyError } = await supabase
      .from('attribution_events')
      .select('sum(reward_amount)::numeric')
      .eq('event_type', eventType)
      .gt('rewarded_at', lastHourStart)
      .not('rewarded_at', 'is', null);

    if (rewardedError) {
      console.warn('[Rewards] getRewardStats error:', rewardedError.message);
    }

    return {
      processed: rewardedData?.[0]?.count || 0,
      totalReward: parseFloat(rewardedData?.[0]?.sum) || 0,
      lastDistributedAt: rewardedData?.[0]?.max_rewarded_at || null,
      pending: pendingData?.[0]?.count || 0,
      lastHourlyVolume: parseFloat(hourlyData?.[0]?.sum) || 0,
    };
  } catch (err) {
    console.warn('[Rewards] getRewardStats exception:', err.message);
    return {
      processed: 0,
      totalReward: 0,
