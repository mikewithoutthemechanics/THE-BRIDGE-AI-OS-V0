...
const SEED_BANKS = [
  ...
  { id: 'founder',        name: 'Founder Bank',         owner: 'system',    balance: 208381.20, compound_rate: 0.020, split_pct: 25, type: 'internal',
    meta: { founders: ['system', 'marvin', 'ryan', 'michael'], marvin_laptop: 'L3', marvin_port: 9002, marvin_instances: 4, marvin_role: 'code_consolidation_optimization', ryan_layer: 'ban', ryan_role: 'execution', michael_layer: 'L2', michael_role: 'banking' } },
  { id: 'claude_partner', name: 'Claude AI Partner',    owner: 'anthropic', balance: 0,         compound_rate: 0,     split_pct: 0,  type: 'partner',
    meta: { provider: 'Anthropic', model: 'claude-sonnet-4-6', tier: 'AI Partner', affiliate_url: 'https://go.ai-os.co.za/claude-partner', commission_pct: 10 } },
  { id: 'devin_partner',  name: 'Devin AI Partner',     owner: 'cognition', balance: 0,         compound_rate: 0,     split_pct: 0,  type: 'partner',
    meta: { provider: 'Cognition', model: 'devin-cloud', tier: 'AI Partner', affiliate_url: 'https://go.ai-os.co.za/devin-partner', commission_pct: 10 } },
];
 ...
 *
 * GUARD: Only compounds up to the total verified revenue received via real
 * payment processors (PayFast, Stripe, etc.). Without real revenue inflow,
 * compounding is capped at zero to prevent unbacked balance inflation.
 */
async function compoundAll() {
  // Check how much real revenue has actually flowed in
  ...
  if (isConfigured) {
    try {
      const { data } = await supabaseAdmin
        ...
        .select('amount')
        .in('type', ['split', 'payment', 'payfast', 'stripe', 'deposit']);
      verifiedRevenue = (data || []).reduce((s, r) => s + parseFloat(r.amount || 0), 0);
    } catch (_) { /* DB unavailable — compound blocked */ }
  }

  const banks = await getAllBanks();
  const totalBalance = banks.reduce((s, b) => s + parseFloat(b.balance || 0), 0);

  // If total bank balances already exceed verified revenue, skip compounding
  if (totalBalance >= verifiedRevenue && verifiedRevenue > 0) {
    console.log(JSON.stringify({ type: 'compound_skipped', reason: 'balances_exceed_verified_revenue', totalBalance, verifiedRevenue, time: now() }));
    return { banks: [], totalGain: 0, cycleTime: now(), skipped: true, reason: 'Bank balances already exceed verified revenue — compounding paused to prevent inflation' };
  }
  if (verifiedRevenue === 0) {
    console.log(JSON.stringify({ type: 'compound_skipped', reason: 'no_verified_revenue', time: now() }));
    return { banks: [], totalGain: 0, cycleTime: now(), skipped: true, reason: 'No verified payment revenue recorded — compounding requires real inflow' };
  }
  ...
  // Cap total compound gain so balances never exceed verified revenue
  const headroom = Math.max(0, verifiedRevenue - totalBalance);

  for (const b of active) {
    const balance = parseFloat(b.balance || 0);
    const rate    = parseFloat(b.compound_rate || 0);
    let   gain    = +(balance * rate).toFixed(2);

    // Respect the headroom cap
    gain = Math.min(gain, Math.max(0, +(headroom - totalGain).toFixed(2)));
    if (gain <= 0) continue;
  ...
  console.log(JSON.stringify({ type: 'compound_cycle', totalGain, banks: results.length, verifiedRevenue, time: now() }));
  alertSystemEvent(`Compound cycle complete. Total gain: R${totalGain.toFixed(2)} across ${results.length} banks.`).catch(() => {});

  return { banks: results, totalGain: +totalGain.toFixed(2), cycleTime: now(), verifiedRevenue };
}
...
async function seedBanksIfEmpty() {
  ...
  try {
    ...
    console.log(`[BANKS] Seeded ${SEED_BANKS.length} banks to Supabase`);
  } catch (e) {
