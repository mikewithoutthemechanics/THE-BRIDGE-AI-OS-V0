...
async function pushTelemetry(api_key, readings = []) {
  ...
  if (isConfigured && readings.length > 0) {
    ...
    try {
      await supabaseAdmin.from('iot_telemetry').insert(rows);
    } catch (e) { console.warn('[iot-agent] telemetry insert:', e.message); }
    ...
    } catch (e) { console.warn('[iot-agent] device update:', e.message); }
