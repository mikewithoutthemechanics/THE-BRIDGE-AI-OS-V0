...
let _getNetRevenue = () => {
  try {
    const pnl = ledger.getSystemPnL();
    return pnl.total_net_profit || 0;
  } catch (_) { return 0; }
};
let _getAgentPerformance = () => {
  try {
...
let _creditAgent = (agentId, amount, reason) => {
  try { ledger.credit(agentId, amount, 'compounding_bonus', reason || 'compounding_bonus'); } catch (_) {}
};
let _addToTreasury = (amount) => {
  try { ledger.credit('treasury', amount, 'treasury_reserve', 'Compounding cycle treasury reserve'); } catch (_) {}
};
let _burnTokens = (amount) => {
  try { ledger.debit('treasury', amount, 'burn', 'Compounding cycle burn'); } catch (_) {}
};
...
function runCycle() {
  let netRevenue;
  try { netRevenue = _getNetRevenue(); } catch (_) { netRevenue = 0; }
  ...
  let agents = [];
  try { agents = _getAgentPerformance(); } catch (_) { agents = []; }
  ...
  if (agents.length > 0 && reinvest > 0) {
    ...
    for (const agent of sorted) {
      ...
      if (bonus > 0) {
        try { _creditAgent(agent.agent_id || agent.id, bonus, 'compounding_bonus'); } catch (_) {}
        topPerformers.push({
  ...
  // Treasury reserve
  try { _addToTreasury(reserve); } catch (_) {}

  // Burn
  try { _burnTokens(burn); } catch (_) {}
