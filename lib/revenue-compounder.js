...
async function runCycle() {
  ...
  // ── Record reserve slice to proof chain ────────────────────────────────
  if (reserve > 0) {
    try {
      const proofStore = require('./proof-store');
      const db = require('./db');
      const cycleRef = 'compound:' + (stats.cycles + 1) + ':' + Date.now();
      await proofStore.recordPayment({
        id: cycleRef,
        amount: reserve,
        currency: 'BRDG',
        source: 'compounding-reserve',
        webhookId: cycleRef,
        webhookSignature: { method: 'compounding_cycle', verified: true },
        timestamp: new Date().toISOString(),
        meta: { cycle: stats.cycles + 1, netRevenue, reinvest, reserve, liquidity, burn },
      });
      await db.addToTreasury(reserve, cycleRef);
    } catch (_) {}
  }
...
function startCompounding() {
  ...
  if (!isConfigured) {
    console.log('[COMPOUNDER] Supabase not configured — compounding disabled (offline mode).');
    return;
  }
  if (compoundingTimer) {
