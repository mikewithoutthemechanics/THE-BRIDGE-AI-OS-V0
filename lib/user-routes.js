...
async function authMiddleware(req, res, next) {
  ...
  const user = await userDb.verifyAuthToken(token);
...
async function optionalAuth(req, _res, next) {
  ...
  const token = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : null;
  if (token) {
    req.user = (await userDb.verifyAuthToken(token)) || null;
  }
  next();
}
...
function registerUserRoutes(app) {
  ...
  app.post('/api/auth/register', [validate.register], async (req, res) => {
    try {
      ...
      const user = await userDb.createUser(email, name, 'email', null, password);
  ...
  app.post('/api/auth/google', [validate.googleAuth], async (req, res) => {
    try {
  ...
  // GET /api/auth/me
  app.get('/api/auth/me', authMiddleware, [validate.authMe], async (req, res) => {
  ...
  app.post('/api/user/page-visit', authMiddleware, [validate.pageVisit], async (req, res) => {
    try {
      const { page } = req.body;
      if (!page) return res.status(400).json({ ok: false, error: 'Page is required' });

      const user = await userDb.recordPageVisit(req.user.id, page);
  ...
  app.post('/api/user/conversation', optionalAuth, [validate.conversation], async (req, res) => {
    try {
      if (!req.user) return res.json({ ok: true, message: 'Anonymous conversation recorded' });

      const user = await userDb.recordConversation(req.user.id);
  ...
  app.get('/api/user/journey', authMiddleware, [validate.userJourney], async (req, res) => {
    const user = await userDb.getUserById(req.user.id);
    if (!user) return res.status(404).json({ ok: false, error: 'User not found' });
  ...
  app.get('/api/nurture/queue', [validate.nurtureQueue], async (req, res) => {
    const queue = await userDb.getNurtureQueue();
    res.json({
      ok: true,
  ...
  app.post('/api/nurture/advance', [validate.nurtureAdvance], async (req, res) => {
    try {
      ...
      if (stage) {
        ...
        const updated = await userDb.updateFunnelStage(user_id, stage);
      ...
      if (currentIdx < userDb.FUNNEL_ORDER.length - 1) {
        ...
        const updated = await userDb.updateFunnelStage(user_id, nextStage);
  ...
  app.get('/api/nurture/funnel', [validate.nurtureFunnel], async (_req, res) => {
    const stats = await userDb.getFunnelStats();
    res.json({ ok: true, ...stats });
  });

  // GET /api/nurture/leads
  app.get('/api/nurture/leads', [validate.nurtureLeads], async (_req, res) => {
    const leads = await userDb.getAllUsers({ funnel_stage: null });
