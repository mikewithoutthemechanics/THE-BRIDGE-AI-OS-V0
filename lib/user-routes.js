...
function registerUserRoutes(app) {
  ...
  app.post('/api/user/page-visit', authMiddleware, async (req, res) => {
    try {
      const { page } = req.body || {};
  ...
  app.post('/api/user/conversation', optionalAuth, async (req, res) => {
    try {
  ...
  app.get('/api/user/journey', authMiddleware, async (req, res) => {
    const user = await userDb.getUserById(req.user.id);
    if (!user) return res.status(404).json({ ok: false, error: 'User not found' });
  ...
  app.get('/api/nurture/queue', async (req, res) => {
  ...
  app.post('/api/nurture/advance', async (req, res) => {
    try {
  ...
  app.get('/api/nurture/funnel', async (_req, res) => {
    const stats = await userDb.getFunnelStats();
    res.json({ ok: true, ...stats });
  });

  // GET /api/nurture/leads
  app.get('/api/nurture/leads', async (_req, res) => {
