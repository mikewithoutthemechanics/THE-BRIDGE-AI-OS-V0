...
function registerEconomyRoutes(app) {
  ...
  // Internal webhook endpoint called by the PayFast payment gateway.
  // Authentication is via x-bridge-secret (BRIDGE_INTERNAL_SECRET env var),
  // not by user JWT — requireAdmin is intentionally absent here.
  ...
  app.get('/api/economy/tasks', async (req, res) => {
    // Map common status names to internal format: open→POSTED, claimed→CLAIMED, done→COMPLETED
    var statusMap = { open: 'POSTED', claimed: 'CLAIMED', done: 'COMPLETED', completed: 'COMPLETED' };
    var rawStatus = req.query.status;
    var mappedStatus = rawStatus ? (statusMap[rawStatus.toLowerCase()] || rawStatus) : undefined;
    const tasks = typeof market.listTasks === 'function'
      ? await market.listTasks(mappedStatus)
      : [];
  ...
  app.get('/api/economy/stats', async (_req, res) => {
    try {
      ...
      // Enrich with active/open task counts from marketplace
      try {
        const openTasks = await market.listTasks('POSTED');
        const claimedTasks = await market.listTasks('CLAIMED');
        stats.activeTasks = (openTasks ? openTasks.length : 0) + (claimedTasks ? claimedTasks.length : 0);
        stats.openTasks = openTasks ? openTasks.length : 0;
        stats.claimedTasks = claimedTasks ? claimedTasks.length : 0;
      } catch (_) {
        stats.activeTasks = 0;
        stats.openTasks = 0;
        stats.claimedTasks = 0;
      }
