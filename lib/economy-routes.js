...
function registerEconomyRoutes(app) {
  ...
  app.get('/api/economy/balance/:agentId', 
    validation.validateRequest({
      agentId: { type: 'string', required: true, validate: validation.validateId }
    }),
    async (req, res) => {
...
// Manual transfer between agents (admin only)
  ...
  app.post('/api/economy/checkout',
    validation.validateRequest({
      email: { type: 'string', required: true, validate: validation.validateEmail },
      brdg_amount: { type: 'positive', required: true },
      item_name: { type: 'string', required: false, max: 200 },
    ...
    async (req, res) => {
      ...
      const zarAmount = brdgNum * zarToBrdg;
  ...
  app.post('/api/economy/tasks/:id/complete', async (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, task, claimer: task.claimer_agent, reward: task.reward });
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  });
