...
function registerEconomyRoutes(app) {
  ...
  app.get('/api/economy/tasks', (req, res) => {
    ...
    const tasks = market.listTasks(status ? { status } : {});
    res.json({ ok: true, tasks, count: tasks.length });
  });

  // Post a new task
  app.post('/api/economy/tasks', (req, res) => {
    const { poster, title, description, reward } = req.body;
    try {
      const task = market.postTask({ poster, title, description, reward: Number(reward) });
