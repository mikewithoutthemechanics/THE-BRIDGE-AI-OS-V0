...
let _limitsTableChecked = false;
async function ensureWithdrawalTable() {
  if (_limitsTableChecked || !supabase) return;
  _limitsTableChecked = true;

  const { error } = await supabase.from('withdrawal_requests').select('id', { head: true, count: 'exact' });
  if (error && error.code === '42P01') {
    console.warn('[withdrawal-limits] withdrawal_requests missing — attempting auto-create...');
    try {
      const createSql = `CREATE TABLE IF NOT EXISTS withdrawal_requests (
      ...
      console.log('[withdrawal-limits] Auto-created withdrawal_requests successfully');
    } catch (e) {
      console.warn('[withdrawal-limits] Auto-create failed:', e.message);
    }
  }
}
