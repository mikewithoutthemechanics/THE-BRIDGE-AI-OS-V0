...
async function delegateTask(primeId, task) {
  const prime = getPrimeAgent(primeId);
  ...
  try {
    ...
    taskResult = await market.postTask(
      primeId,
      '[' + prime.name + '] ' + title,
      description || 'Delegated by Prime Agent ' + prime.name + ' (' + prime.title + ')',
      rewardBrdg,
    ...
    if (taskResult && taskResult.id) {
      try {
        await market.claimTask(taskResult.id, targetAgent);
...
async function primeThink(primeId, question) {
  ...
  try {
    ...
    const result = await llm.infer(question, {
      system: systemPrompt,
      max_tokens: 1024,
    });
...
async function primeReport(primeId) {
  ...
  try {
    ...
    economyData = await ledger.getStats();
    // Get balances for subordinates
    const subBalances = await Promise.all(prime.subordinates.map(async (id) => {
      try {
  ...
  try {
    const market = require('./task-market');
    taskData = await market.getMarketStats();
  } catch (_) {
  ...
  try {
    const llm = require('./llm-client');
    const result = await llm.infer('Generate your domain status report for Bridge AI OS. Be specific and actionable.', {
      system: systemPrompt,
      max_tokens: 1500,
    });
...
async function runPrimeLoop() {
  ...
  try {
    const ledger = require('./agent-ledger');
    for (const p of PRIME_AGENTS) {
      try { await ledger.ensureAgent(p.id); } catch (_) {}
    }
  } catch (_) {}
