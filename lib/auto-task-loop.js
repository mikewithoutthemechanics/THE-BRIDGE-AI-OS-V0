...
const OPS_FLOOR = 50000;   // refill threshold
const OPS_TARGET = 500000; // refill target

/**
 * Self-healing: ensure the ops bank has enough balance to fund tasks.
 * Called at the start of each generate cycle.
 */
async function ensureOpsBank() {
  try {
    const ops = await banks.getBank('ops');
    const bal = parseFloat(ops?.balance || 0);
    if (bal < OPS_FLOOR) {
      const topUp = OPS_TARGET - bal;
      await banks.creditBank('ops', topUp, 'auto_refill', 'Ops bank auto-refill — floor breached');
      console.log('[AUTO-ECON] Ops bank auto-refilled +' + topUp.toFixed(0) + ' BRDG → ' + OPS_TARGET);
    }
  } catch (e) {
    console.warn('[AUTO-ECON] ensureOpsBank error:', e.message);
  }
}
  ...
  try {
    ...
    for (var i = 0; i < claimed.length; i++) {
      ...
      if (elapsed > 5000) { // >5 seconds — Config E: fast settlement
        try {
          ...
          var s1 = (settlement && settlement.settlement) || { claimer_paid: 0, burned: 0, treasury_fee: 0 };
          console.log(
            '[AUTO-ECON] Task completed: "' + task.title + '" by ' + task.claimer_agent +
            ' | paid: ' + (s1.claimer_paid || 0).toFixed(2) +
            ' BRDG | burned: ' + (s1.burned || 0).toFixed(2) + ' BRDG'
          );
          stats.tasks_completed++;
          stats.total_brdg_moved += s1.claimer_paid || 0;
          stats.total_burned += s1.burned || 0;
          completed++;

          // Recycle treasury fee back to ops bank to prevent drain
          var treasuryFee = s1.treasury_fee || 0;
          if (treasuryFee > 0) {
            try {
    ...
    for (var j = 0; j < executing.length; j++) {
      ...
      try {
        ...
        var s2 = (exSettlement && exSettlement.settlement) || { claimer_paid: 0, burned: 0, treasury_fee: 0 };
        console.log(
          '[AUTO-ECON] Task completed: "' + exTask.title + '" by ' + exTask.claimer_agent +
          ' | paid: ' + (s2.claimer_paid || 0).toFixed(2) +
          ' BRDG | burned: ' + (s2.burned || 0).toFixed(2) + ' BRDG'
        );
        stats.tasks_completed++;
        stats.total_brdg_moved += s2.claimer_paid || 0;
        stats.total_burned += s2.burned || 0;
        completed++;

        // Recycle treasury fee back to ops bank
        var exTreasuryFee = s2.treasury_fee || 0;
        if (exTreasuryFee > 0) {
          try {
...
function startAutoLoop(opts) {
  if (!isConfigured) {
    console.log('[AUTO-ECON] Supabase not configured — economy loop disabled (offline mode).');
    return stats;
  }
  if (_running) {
