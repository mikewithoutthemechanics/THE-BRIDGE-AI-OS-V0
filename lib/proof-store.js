...
/**
 * Diagnose chain integrity issues and attempt repair.
 * Returns detailed report of broken transactions.
 */
async function diagnoseChain(limit = 1000) {
  const proofs = await getAllProofs(limit);
  const issues = [];
  let prevHash = '0'.repeat(64);
  ...
  for (let i = 0; i < proofs.length; i++) {
    ...
    // Only advance if this transaction is valid
    if (p.previous_hash === prevHash && recomputed === p.tx_hash) {
      prevHash = p.tx_hash;
    }
  }
  ...
  return {
    ...
    firstBrokenAt: issues.length > 0 ? issues[0].index : null,
    chainHealth: issues.length === 0 ? 'healthy' : 'broken',
  };
}

/**
 * Rebuild the chain from a specific transaction onwards.
 * WARNING: This recalculates hashes and updates the database.
 * Only call this if you understand the implications.
 */
async function rebuildChainFrom(startIndex = 0) {
  if (!isConfigured) return { error: 'Supabase not configured' };

  const proofs = await getAllProofs(10000);
  if (proofs.length === 0) return { rebuilt: 0 };

  let prevHash = startIndex === 0 ? '0'.repeat(64) : proofs[startIndex - 1].tx_hash;
  const updated = [];

  for (let i = startIndex; i < proofs.length; i++) {
    const p = proofs[i];

    // Recompute correct hash using the full proof data
    const newProof = zt.createPaymentProof({
      id: p.transaction_id,
      amount: p.amount,
      currency: p.currency,
      source: p.source,
    ...
    // Update database with corrected hashes and signature
    try {
      await supabaseAdmin
        .from('payment_proofs')
        .update({
          tx_hash: newProof.txHash,
          previous_hash: prevHash,
          proof_signature: newProof.proofSignature,
        })
        .eq('transaction_id', p.transaction_id);

      updated.push(p.transaction_id);
      prevHash = newProof.txHash;
    } catch (e) {
      console.warn('[proof-store] rebuild failed at', p.transaction_id, ':', e.message);
      break;
    }
  }

  return {
    rebuilt: updated.length,
