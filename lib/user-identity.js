...
// ── In-memory fallback store (active when Supabase is not configured) ────────
// Allows tests and dev environments to run without infrastructure.
const _mem = {
  users:    new Map(), // id → user
  byEmail:  new Map(), // email.toLowerCase() → id
  byApiKey: new Map(), // apiKey → id
  wallets:  new Map(), // userId → [{wallet_address, chain, ...}]
};
...
async function createUser(email, name, provider, oauthId, passwordRaw) {
  ...
  if (!isConfigured) {
    const existingId = _mem.byEmail.get(email);
    if (existingId) return _mem.users.get(existingId);
    const id = uuid();
    const ts = now();
    const apiKey = 'brdg_' + crypto.randomBytes(24).toString('hex');
    const user = {
      id, email, name: name || null,
      oauth_provider: provider || 'email', oauth_id: oauthId || null,
      password_hash: passwordRaw ? hashPassword(passwordRaw) : null,
      api_key: apiKey, first_seen: ts, last_seen: ts,
      plan: 'free', funnel_stage: 'identified', lead_score: 0,
      brdg_balance: 0, pain_points: [], pages_visited: [],
      conversations: 0, role: 'user', totp_enabled: false,
    };
    _mem.users.set(id, user);
    _mem.byEmail.set(email, id);
    _mem.byApiKey.set(apiKey, id);
    return user;
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      id, email, name: name || null,
      oauth_provider: provider || 'email', oauth_id: oauthId || null,
      password_hash: passwordHash, api_key: apiKey,
      first_seen: ts, last_seen: ts, plan: 'free',
      funnel_stage: 'identified', lead_score: 0, brdg_balance: 0,
      pain_points: [], pages_visited: [], conversations: 0,
...
async function getUserByEmail(email) {
  ...
  email = email.toLowerCase().trim();
  if (!isConfigured) {
    const id = _mem.byEmail.get(email);
    return id ? _mem.users.get(id) || null : null;
  }
  const { data, error } = await supabase
...
async function getUserById(id) {
  if (!id) return null;
  if (!isConfigured) return _mem.users.get(id) || null;
  const { data, error } = await supabase
...
async function getUserByApiKey(apiKey) {
  ...
  if (!isConfigured) {
    const id = _mem.byApiKey.get(apiKey);
    return id ? _mem.users.get(id) || null : null;
  }
  const { data, error } = await supabase
    .from('users').select('*').eq('api_key', apiKey).single();
  if (error && error.code === 'PGRST116') return null;
...
async function updateUser(userId, fields) {
  ...
  for (const [k, v] of Object.entries(fields)) {
    if (allowed.includes(k) && v !== undefined) updates[k] = v;
  }
  if (Object.keys(updates).length === 0) return getUserById(userId);
  updates.last_seen = now();
  if (!isConfigured) {
    const user = _mem.users.get(userId);
    if (!user) return null;
...
async function updateFunnelStage(userId, stage) {
  ...
  if (!isConfigured) {
    const user = _mem.users.get(userId);
    if (user) { user.funnel_stage = stage; user.last_seen = now(); }
    return user || null;
  }
  const { data, error } = await supabase
    .from('users')
...
async function updateLeadScore(userId, delta) {
  ...
  if (!isConfigured) {
    user.lead_score = newScore; user.last_seen = now();
    return user;
  }
  const { data, error } = await supabase
    .from('users')
