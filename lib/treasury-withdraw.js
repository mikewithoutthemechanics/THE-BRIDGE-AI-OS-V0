...
// ── BRDG Swap Module ─────────────────────────────────────────────────────────
let brdgSwap;
try { brdgSwap = require('./brdg-swap'); } catch (_) { brdgSwap = null; }
...
const RAILS = {
  ...
  brdg_to_eth: {
    name: 'BRDG → ETH Conversion', chain: 'linea', type: 'dex',
    fee_pct: 0.003, fee_flat: 0.0001, min: 10,
    pipeline: ['brdg_swap'],
    description: 'Swap BRDG to ETH via SyncSwap on Linea',
  },
  defi: {
...
async function ensureWithdrawalTables() {
  if (_tablesChecked || !supabase) return;
  _tablesChecked = true;
  ...
  for (const tbl of tablesToCheck) {
    try {
      const { error } = await supabase.from(tbl.name).select('*', { head: true, count: 'exact' });
      if (error && error.code === '42P01') {
        console.warn(`[treasury-withdraw] ${tbl.name} missing — attempting auto-create...`);
        try {
          const { error: rpcErr } = await supabase.rpc('exec_sql', { sql: tbl.sql });
          if (rpcErr) throw rpcErr;
          console.log(`[treasury-withdraw] Auto-created ${tbl.name} successfully`);
        } catch (createErr) {
          console.warn(`[treasury-withdraw] Auto-create ${tbl.name} failed:`, createErr.message);
        }
      }
    } catch (_) {}
  }
...
async function executeWithdrawal(params) {
  ...
  try {
    for (const step of pipelineSteps) {
      ...
      } else if (step === 'brdg_swap') {
        // BRDG → ETH swap via SyncSwap
        if (!brdgSwap) {
          return { ok: false, error: 'brdg-swap module not available — install ethers', pipeline: pipelineResults };
        }
        try {
          const result = await brdgSwap.swapBRDGtoETH(String(computation.net), to);
          ...
          pipelineResults.push({ 
            ...
            from: 'BRDG', 
            to_currency: 'ETH', 
            amount: computation.net,
            eth_out: result.ethOut || result.ethOutActual,
          });
        } catch (swapError) {
          return { ok: false, error: `BRDG→ETH swap failed: ${swapError.message}`, pipeline: pipelineResults };
        }
