...
async function stageActivation() {
  ...
  try {
    ...
    log('activation', `Processed ${sent} due touches (${result?.due || 0} were due)`);
  } catch (e) {
...
async function stageAppLoop() {
  // Emit CRM signals from 50 active apps — closes Module 1 + 8 + 9
  try {
    const lc = require('./loop-closure');
    const emitted = await lc.emitAppCrmSignals();
    if (emitted > 0) {
      log('app_loop', `Emitted ${emitted} app→CRM signals`);
    }
  } catch (_) {}
}
...
async function tick() {
  ...
  try {
    ...
    // Every 5th tick: emit app→CRM signals (Module 1/8/9 loop closure)
    if (state.ticks % 5 === 0) {
      await stageAppLoop();
    }
