...
function _masterSecret() {
  ...
  if (!sources.length) {
    throw new Error(
      ...
      'Set BRIDGE_SIWE_JWT_SECRET (≥32 chars).'
    );
  }
...
function _deriveKey(purpose, agentId) {
  ...
  return crypto.createHmac('sha256', master)
    .update(`${purpose}:${agentId}`)
    .digest(); // 32 bytes → valid secp256k1 private key
}
...
/**
 * ethers.Wallet connected to the Linea provider for an agent.
 * Use this for signing transactions on behalf of the agent.
...
function getEthWallet(agentId) {
  ...
  return new ethers.Wallet('0x' + key.toString('hex'), provider);
}
