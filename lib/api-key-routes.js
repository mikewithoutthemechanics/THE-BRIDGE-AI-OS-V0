...
function registerApiKeyRoutes(app) {
  ...
  app.post('/api/keys/create', (req, res) => {
    try {
      ...
      const result = apiKeys.createKey(email, plan || 'starter');
  ...
  app.get('/api/keys/usage', (req, res) => {
    try {
      ...
      const usage = apiKeys.getUsage(apiKey);
      if (!usage) {
  ...
  app.get('/api/keys/validate', (req, res) => {
    try {
      ...
      const data = apiKeys.validateKey(apiKey);
      if (!data) {
        return res.status(401).json({ error: 'Invalid API key' });
      }
   ...
   */
  app.post('/api/keys/topup', (req, res) => {
    try {
      const apiKey = extractApiKey(req);
      if (!apiKey) {
        return res.status(401).json({ error: 'Missing API key. Provide X-API-Key header or Authorization: Bearer <key>' });
      ...
      const result = apiKeys.addCredits(apiKey, brdgAmount);
