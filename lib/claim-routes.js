...
async function ensureClaimsTable() {
  ...
  if (error && error.code === '42P01') {
    console.warn('[claim-routes] agent_claims table missing — attempting auto-create...');

    // Try to create via Supabase admin (service role)
    try {
      ...
      if (supabaseAdmin) {
        ...
        `;
        const { error: rpcError } = await supabaseAdmin.rpc('exec_sql', { sql: createSql });
        if (rpcError) throw rpcError;
        console.log('[claim-routes] Auto-created agent_claims table successfully');
      }
    } catch (createErr) {
      console.warn('[claim-routes] Auto-create failed:', createErr.message);
      console.warn('[claim-routes] Run manually: node migrations/apply-withdrawal-system.js');
    }
  }
}
