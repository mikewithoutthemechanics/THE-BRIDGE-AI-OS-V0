...
const PROVIDERS = {
  kilo: {
    ...
    envKey: 'KILO_API_KEY',
    // Default: free tier. Set KILO_MODEL=anthropic/claude-sonnet-4.6 to upgrade.
    // Use opts.premium=true in infer() to route a single call to Claude at no extra key cost.
    model: process.env.KILO_MODEL || 'kilo-auto/free',
...
function saveUsage() {
  // Debounce: batch concurrent writes into a single flush after 1 second
...
async function callKilo(apiKey, prompt, opts = {}) {
  // opts.premium=true → use Claude claude-sonnet-4-6 via Kilo (same key, no extra cost)
  const model = opts.model || (opts.premium ? PROVIDERS.kilo.premiumModel : PROVIDERS.kilo.model);
...
async function callOpenRouter(apiKey, prompt, opts = {}) {
  ...
  const res = await httpPost('https://openrouter.ai/api/v1/chat/completions', {
    'Authorization': `Bearer ${apiKey}`,
    'HTTP-Referer': 'https://bridge-ai-os.com',
