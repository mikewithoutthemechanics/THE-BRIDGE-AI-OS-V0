...
// withCircuitBreaker(fn, threshold, resetMs) binds `fn` at creation.
// We use a mutable holder so each invocation can swap the real work in.
const agentBreakers = new Map();
function getBreaker(agentName) {
  if (!agentBreakers.has(agentName)) {
    const holder = { fn: null };
    const breaker = resilience.withCircuitBreaker(
      () => holder.fn(),
      3,    // failureThreshold — 3 consecutive failures opens the circuit
      30000 // resetTimeout — 30 s before half-open probe
    );
    agentBreakers.set(agentName, { breaker, holder });
  }
  return agentBreakers.get(agentName);
}
...
class Orchestrator {
  ...
  async executeSingle(agentName, input, sessionId) {
    ...
    try {
      // Wire circuit breaker around the retry+timeout stack
      const { breaker, holder } = getBreaker(agentName);
      holder.fn = async () => {
      ...
      const cbOut = await breaker.execute();
      // withCircuitBreaker.execute returns { result, circuitState, failures }
      const retryOut = cbOut.result !== undefined ? cbOut.result : cbOut;
      ...
      const envelope = contract.success(agentName, result, {
      ...
      // H4: validate the response envelope — warn but don't block (graceful degradation)
      const validation = contract.validate(envelope);
      if (!validation.valid) {
        console.warn('[AP2-v3] Contract validation warning for agent "' + agentName + '":', validation.errors.join('; '));
      }
  ...
  async executeChain(agentNames, input, sessionId) {
    ...
    if (chainAborted) {
      const failedStep = chain[chain.length - 1];
      return contract.error('orchestrator',
        'Chain aborted at agent "' + (failedStep?.agent || 'unknown') + '" (step ' + chain.length + '/' + agentNames.length + ')',
        'CHAIN_ABORTED',
        chainMeta
      );
    }
