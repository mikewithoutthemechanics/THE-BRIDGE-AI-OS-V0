# Direct VPS commands - copy and paste each line on your VPS
...
# 4. Create Blackbox exporter config
cat > monitoring/prometheus/blackbox.yml << 'EOF'
modules:
  http_2xx:
    prober: http
    timeout: 5s
    http:
      ...
      method: GET
      headers:
        User-Agent: "Bridge AI OS - Domain Monitor"
EOF
...
docker run -d --name blackbox-exporter --restart unless-stopped -p 9115:9115 -v $(pwd)/monitoring/prometheus/blackbox.yml:/etc/blackbox_exporter/config.yml prom/blackbox-exporter
docker run -d --name prometheus --restart unless-stopped -p 9090:9090 -v $(pwd)/monitoring/prometheus:/etc/prometheus prom/prometheus

echo "✅ All monitoring services started!"
echo ""
echo "🎉 Bridge AI OS monitoring is now live!"
echo ""
echo "📊 Access your monitoring dashboards:"
echo "   http://your-vps-ip:9090 (Prometheus)"
echo "   https://bridge-ai-os.com/monitoring (Live Dashboard)"
echo "   https://ai-os.co.za/monitoring (Live Dashboard)"
echo ""
