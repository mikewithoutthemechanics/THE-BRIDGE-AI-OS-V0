...
(function PHERE() {
  ...
  var themeColors = {
    ...
    user: {
      primaryRgb: '99,255,218',
      secondaryRgb: '0,229,123',
      tertiaryRgb: '168,85,247',
      glow: 'rgba(99,255,218,0.08)'
    }
  };
        ...
        for (var a = 0; a < particles.length; a++) {
          for (var b = a + 1; b < particles.length; b++) {
            ...
            if (dist < THRESH) {
              ...
              ctx.strokeStyle = 'rgba(' + c.primaryRgb + ',' + ((1 - dist / THRESH) * 0.08).toFixed(3) + ')';
              ctx.lineWidth = 0.5;
              ctx.stroke();
            }
          }
  ...
  function initKineticType() {
    ...
    var headings = document.querySelectorAll('h1, h2, h3, .phere-kinetic');
    headings.forEach(function (h) {
      ...
      chars.forEach(function (ch, i) {
        ...
        span.dataset.phereIndex = String(i);
        h.appendChild(span);
      });
      h.addEventListener('mouseenter', function () {
        h.querySelectorAll('span').forEach(function (s) {
          var idx = parseInt(s.dataset.phereIndex, 10);
          var delay = idx * 20;
          setTimeout(function () {
  ...
  window.BRIDGE_PHERE = {
    ...
    injectWordPress: function () {
      ...
      fetch('/wp-json/bridge-ai/v1/phere/activate', {
        ...
        body: JSON.stringify({ version: '1.0.0', tier: PERF.tier, theme: theme }),
      }).catch(function () {});
    },
