(function() {
  ...
  var sections = {
    ...
    'SYSTEM': [
      ...
      { name: 'TREASURY', url: svcBase+'/treasury-dashboard.html' }, { name: 'BANKS', url: svcBase+'/banks.html' },
      { name: 'INFRA', url: svcBase+'/infra.html' }, { name: 'DASHBOARD', url: svcBase+'/aoe-dashboard.html' }
    ],
    'MORE': [
      ...
      { name: 'PRICING', url: svcBase+'/pricing.html' }, { name: 'PAYMENT', url: svcBase+'/payment.html' },
      { name: 'DOCS', url: svcBase+'/docs.html' }, { name: 'AVATAR', url: svcBase+'/avatar.html' },
  ...
  var css = [
    '.bn-bar{position:fixed;top:0;left:0;right:0;height:40px;z-index:99999 !important;background:' + c.bg1 + ';display:flex;align-items:center;padding:0 8px;font-family:"JetBrains Mono",monospace;border-bottom:1px solid ' + c.primaryGlow + ';box-shadow:0 2px 12px rgba(0,0,0,0.4);gap:6px;pointer-events:all !important;}',
    '.bn-logo{color:' + c.primary + ';font-size:12px;font-weight:700;letter-spacing:2px;white-space:nowrap;cursor:pointer;flex-shrink:0;}',
    ...
    '.bn-pod-sep{width:1px;height:20px;background:' + c.primaryGlow + ';flex-shrink:0;}',
    /* sections */
    ...
    '.bn-group-btn{color:' + c.primary + ';font-size:10px;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;padding:6px 12px;border-radius:4px;cursor:pointer;border:1px solid ' + c.primaryGlow + ';background:' + c.primaryGlow + ';white-space:nowrap;transition:all 0.2s;pointer-events:all;position:relative;z-index:9999;}',
    '.bn-group-btn:hover,.bn-group.open .bn-group-btn{background:' + c.primaryGlow + ';border-color:' + c.primaryGlow + ';}',
    '.bn-dropdown{display:none;position:absolute;top:100%;left:0;margin-top:4px;background:' + c.bg2 + ';border:1px solid ' + c.primaryGlow + ';border-radius:6px;padding:6px;min-width:160px;box-shadow:0 8px 30px rgba(0,0,0,0.8);z-index:100000;}',
    '.bn-group.open .bn-dropdown{display:block;}',
    '.bn-link{display:block;color:#94a3b8;text-decoration:none;font-size:10px;font-weight:500;letter-spacing:1px;text-transform:uppercase;padding:6px 10px;border-radius:3px;transition:all 0.15s;white-space:nowrap;}',
    '.bn-link:hover{color:#e2e8f0;background:' + c.primaryGlow + ';}',
    '.bn-link.bn-active{color:' + c.primary + ';background:' + c.primaryGlow + ';}',
    ...
    '.bn-fab-btn{width:48px;height:48px;border-radius:50%;border:2px solid ' + c.primary + ';background:#050a0f;color:' + c.primary + ';font-size:18px;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 0 14px ' + c.primaryGlow + ';transition:all .2s;}',
    '.bn-fab-btn:hover{background:#0a1a1a;box-shadow:0 0 24px ' + c.primaryGlow + ';}',
    '.bn-fab-panel{display:none;background:#060810;border:1px solid ' + c.primaryGlow + ';border-radius:10px;padding:12px;width:200px;box-shadow:0 8px 32px rgba(0,0,0,0.7);}',
    '.bn-fab-panel.open{display:block;}',
    '.bn-fab-title{color:' + c.primary + ';font-size:9px;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid ' + c.primaryGlow + ';}',
    ...
    '.bn-fab-link.twin{background:' + c.primaryGlow + ';color:#63ffda;border:1px solid ' + c.primaryGlow + ';}',
    '.bn-fab-link.twin:hover{background:' + c.primaryGlow + ';border-color:#63ffda;}',
    ...
    /* ── Mobile hamburger ─────────────────────────────────────────── */
    '.bn-hamburger{display:none;background:none;border:1px solid ' + c.primaryGlow + ';color:' + c.primary + ';font-size:18px;padding:4px 8px;border-radius:4px;cursor:pointer;flex-shrink:0;line-height:1;}',
    '.bn-hamburger:hover{background:' + c.primaryGlow + ';}',
    ...
    '  .bn-group{width:100%;}',
    '  .bn-group-btn{width:100%;text-align:left;padding:12px 14px;font-size:11px;border-radius:6px;border:1px solid ' + c.primaryGlow + ';margin-bottom:2px;}',
